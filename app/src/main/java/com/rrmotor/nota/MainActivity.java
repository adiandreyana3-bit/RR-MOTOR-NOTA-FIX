...
public class MainActivity extends Activity {
    ...
    private void tambahBarisItem() {
        ...
        jumlah.setOnFocusChangeListener((v, hasFocus) -> {
            if (!hasFocus) hitungTotal();
        });

        harga.setOnFocusChangeListener((v, hasFocus) -> {
            if (!hasFocus) hitungTotal();
        });

        hapus.setOnClickListener(v -> {

            int posisi = namaBarang.indexOf(nama);
    ...
    private long hitungTotal() {
        ...
        sisaText.setText("SISA : " + formatRupiah(sisa));
    ...
    private void simpanNota() {
        ...
        if (nama.isEmpty()) {
            namaInput.setError("Nama wajib diisi");
            namaInput.requestFocus();
            return;
        }

        if (wa.isEmpty()) {
            waInput.setError("Nomor WhatsApp wajib diisi");
            waInput.requestFocus();
            return;
        }

        if (tanggal.isEmpty()) {
            tanggalInput.setError("Tanggal wajib diisi");
            return;
        }
        ...
        for (int i = 0; i < namaBarang.size(); i++) {
            ...
            String barang = namaBarang.get(i)
                    .getText().toString()
                    .trim();
        ...
        String data =
                ...
                encode(wa) + "|" +
                encode(tanggal) + "|" +
                encode(motorInput.getText().toString()) + "|" +
                angka(dpInput) + "|" +
                dataItem;
        ...
        String riwayat = pref.getString(KEY_HISTORY, "");
    ...
    private void tampilkanRiwayat() {
        ...
        String data = pref.getString(KEY_HISTORY, "");

        if (data.isEmpty()) {
            new AlertDialog.Builder(this)
                    .setTitle("📋 Riwayat Nota")
                    .setMessage("Belum ada nota tersimpan.")
                    .setPositiveButton("OK", null)
                    .show();
            return;
        }

        String[] semua = data.split("\n");

        StringBuilder tampilan =
                new StringBuilder();

        for (int i = semua.length - 1; i >= 0; i--) {

            try {

                String[] bagian = semua[i].split("\\|", 7);

                if (bagian.length < 7) continue;

                String nama = decode(bagian[1]);
                String wa = decode(bagian[2]);
                String tanggal = decode(bagian[3]);

                long total = 0;

                String[] items = bagian[6].split(";");
                ...
                for (String item : items) {
                    ...
                    if (isi.length >= 3) {
                        ...
                        long jumlah =
                                Long.parseLong(isi[1]);

                        long harga =
                                Long.parseLong(isi[2]);
        ...
        teks.setText(tampilan.toString());
        teks.setTextSize(16);
    ...
    private void kirimWhatsApp() {
        ...
        pesan.append("Berikut informasi nota servis Anda:\n\n");
        ...
        for (int i = 0; i < namaBarang.size(); i++) {
            ...
            if (!barang.isEmpty()) {
                ...
                pesan.append("   = ")
                        .append(formatRupiah(jumlah * harga))
                        .append("\n\n");
            }
        }
        ...
        if (sisa == 0) {
            pesan.append("✅ *STATUS: LUNAS*\n\n");
        } else {
            pesan.append("⚠️ *STATUS: BELUM LUNAS*\n\n");
        }

        pesan.append("Terima kasih sudah mempercayakan kendaraan Anda kepada *RR MOTOR*. 🙏");

        String nomor = wa.replaceAll("[^0-9]", "");
    ...
    private void cetakNota() {
        ...
        Toast.makeText(
                this,
                "Fitur cetak akan kita pasang berikutnya.",
                Toast.LENGTH_LONG
        ).show();
    }
}
